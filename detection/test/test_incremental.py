...
def main():
    ...
    '''
    When the slope of the function is too small(e.g. k = 0.005), KSWIN cannot detect it,but ADWIN can.
    '''
    k = 0.05

    # Simulate an incremental drift. Variable k is the slope(or velocity) of the drift.
    for i in range(700):
        data_stream[i] = data_stream[i] + k * 700
    for i in range(700, 1400):
        data_stream[i] = data_stream[i] + k * i
    for i in range(1400, 2000):
        data_stream[i] = data_stream[i] + k * 1399
